...
model = torch.hub.load('ultralytics/yolov5', 'custom', path= homedir + "/catkin_ws/src/hector-quadrotor-noetic/hector_quadrotor/hector_quadrotor_gazebo/launch/src/bestPizza500.pt", force_reload=True)
model.to(device)
pub = None
...
def img_callback(data):
    ...
    yolov5_detection.run_detection(real_image, model, pub, 4)
    # print("Camera booted!")
    cv2.imshow("Raw Image 4", cv_image)
    cv2.waitKey(3)
    rate.sleep()

def main():
    print("Camera feed starting up.....")
    rospy.init_node('object_detection4')
    global pub 
    pub = rospy.Publisher('/uav4/found_bb8', Bool, queue_size=5)
    pub.publish(Bool(False))
    img_sub = rospy.Subscriber("/uav4/front_cam/camera/image", Image, img_callback)
    rospy.spin()
