...
def img_callback(data):
    ...
    yolov5_detection.run_detection(real_image, model, pub, 3)
    # print("Camera booted!")
    cv2.imshow("Raw Image 3", cv_image)
    cv2.waitKey(3)
    rate.sleep()

def main():
    print("Camera feed starting up.....")
    rospy.init_node('object_detection3')
    global pub 
    pub = rospy.Publisher('/uav3/found_bb8', Bool, queue_size=5)
    pub.publish(Bool(False))
    img_sub = rospy.Subscriber("/uav3/front_cam/camera/image", Image, img_callback)
    rospy.spin()
