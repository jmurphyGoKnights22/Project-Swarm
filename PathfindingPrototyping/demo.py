...
# choose whether to use my theta star instead of the default A*
USE_THETA_STAR_INSTEAD = False
...
class MainObject:
    ...
    def _setupgui(self):
        ...
        label = Label(astarframe, text="A*", font=("Helvetica", 13), fg='#00FF00')
        label.pack()

        self.pathlabelvar_theta = StringVar()
        label = Label(astarframe, fg='#0000FF', textvariable=self.pathlabelvar_theta)
        label.pack()

        self.astarlabelvar_theta = StringVar()
        label = Label(astarframe, fg='#8080FF', textvariable=self.astarlabelvar_theta)
        label.pack()

        label = Label(astarframe, text="Theta*", font=("Helvetica", 13), fg='#CFCF00')
        label.pack()
    ...
    def onMouseButton1Press(self, event):
        ...
        path_theta, distances_theta, considered_theta = thetastar.thetastar(adjacent, graph.euclidian, graph.euclidian, start, goal, self.quadtree)
        ...
        self.astarlabelvar_theta.set("Nodes visited: {} considered: {}".format(len(distances_theta), considered_theta))
        for tile in distances:
            fill_tile(draw, tile, color=(0xC0, 0xC0, 0xFF))

        if path:
            self.pathlabelvar.set("Path Cost: {}  Nodes: {}".format(round(distances[goal], 1), len(path)))
            path_line_start = path[0]
            path_line_end = path[0]
            for tile in path:
                path_line_start = path_line_end
                path_line_end = tile
                fill_tile(draw, tile, color=(0, 0, 255))
                draw.line(
                [(path_line_start.bb.center()[0], path_line_start.bb.center()[1]),
                (path_line_end.bb.center()[0], path_line_end.bb.center()[1])],
                fill = (0, 255, 0), width = 3)
        else:
            self.pathlabelvar.set("No Path found.")

        if path_theta:
            self.pathlabelvar_theta.set("Path Cost: {}  Nodes: {}".format(round(distances_theta[goal], 1), len(path_theta)))
            path_line_start = path[0]
            path_line_end = path[0]
            for tile in path_theta:
                path_line_start = path_line_end
                path_line_end = tile
                draw.line(
                [(path_line_start.bb.center()[0], path_line_start.bb.center()[1]),
            ...
            self.pathlabelvar_theta.set("No Path found.")
